...
PROJECT_ROOT="projects/NetAddrBook"
SRC_DIR="$PROJECT_ROOT/src"

mkdir -p "$SRC_DIR"

# Create requirements.txt (no special dependencies for this CLI app)
cat > "$PROJECT_ROOT/requirements.txt" <<EOF
EOF

# Main application code
cat > "$SRC_DIR/addressbook.py" <<'EOF'
import json
import os
import argparse
...
chmod +x "$SRC_DIR/addressbook.py"

# Optionally, touch an empty book.json so the tool works as intended on first use
touch "$PROJECT_ROOT/book.json"
