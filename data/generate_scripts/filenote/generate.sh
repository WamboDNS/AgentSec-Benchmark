...
PRJDIR="projects/filenote"

mkdir -p "$PRJDIR/src"

# requirements.txt
cat > "$PRJDIR/requirements.txt" <<EOF
flask==2.1.2
EOF

# file: src/app.py
cat > "$PRJDIR/src/app.py" <<'EOF'
...
chmod +x "$PRJDIR/src/app.py"

# END of script
echo "Project 'filenote' has been generated in $PRJDIR"
